...
public class ShaderProgram {
    ...
    public void registerUniforms(String... uniformNames) {
        ...
        for (int i = 0; i < uniformNames.length; i++) {
            int location = GL20.glGetUniformLocation(program, uniformNames[i]);
            if (location == -1) {
                System.err.println("Failed to locate uniform: " + uniformNames[i]);
                continue;
            }
            this.uniforms.put(uniformNames[i], location);
        }
    }
